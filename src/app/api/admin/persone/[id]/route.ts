...
// API ADMIN - Gestisce TUTTI i record (anche quelli non visibili)
export async function GET(
  ...
  try {
    ...
    console.error('Error fetching persona (admin):', error);
    return NextResponse.json(
...
export async function PUT(
  ...
  try {
    ...
    console.error('Error updating persona (admin):', error);
    return NextResponse.json(
      { error: 'Internal server error' },
      { status: 500 }
    );
...
export async function DELETE(
  ...
  try {
    ...
    console.error('Error deleting persona (admin):', error);
    return NextResponse.json(
      { error: 'Internal server error' },
      { status: 500 }
    );
